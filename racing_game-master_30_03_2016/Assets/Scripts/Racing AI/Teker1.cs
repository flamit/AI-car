...
public class Teker1 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.UpArrow) && Cekis && gaz < tork)
            gaz += tork * Time.deltaTime;
        else if (Cekis)
        {
        ...
        {
            frensayac += 100 * Time.deltaTime;
            if (frensayac < 100)
                fren += 5000 * Time.deltaTime;
            if (frensayac > 100)
            {
                gaz = -600;
                fren = 0;
        ...
        {
            frensayac = 0;
            fren = fren - (fren * Time.deltaTime * 10);
            if (gaz < 0)
                gaz = gaz + (50 * Time.deltaTime);
        }
        ...
        //Debug.Log("gaz="+gaz);
        if (arka)
            GetComponent<WheelCollider> ().steerAngle = -don;
        else
            GetComponent<WheelCollider> ().steerAngle = don;
